...
class AbstractApiModule extends AbstractModule {
  ...
  requestHandler() {
    return async (req, res, next) => {
      ...
      if(method !== 'get') this.log('debug', `API_${func.name.toUpperCase()}`, !Array.isArray(data) ? data._id.toString() : req.apiData.query, req.auth.user._id.toString());
  ...
  queryHandler() {
    return async (req, res, next) => {
      try {
        ...
        req.apiData.query = await this.parseQuery(req.apiData.schemaName, req.body, req.apiData.query, mongoOpts);
   ...
   * @param {Object} options Any extra options
