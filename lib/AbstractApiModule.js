...
class AbstractApiModule extends AbstractModule {
  ...
  async queryHandler (req, res, next) {
    try {
      ...
      // query is already parsed and access-filtered by accessQueryHook; re-validating it here would
      // strip schema-foreign $or clauses (e.g. _isShared) and neuter the access filter
      let results = await this.find(req.apiData.query, { ...opts, validate: false }, mongoOpts)
  ...
  async setUpPagination (req, res, mongoOpts) {
    ...
    const docCount = await mongodb.count(req.apiData.collectionName, req.apiData.query)
  ...
  async checkAccess (req, data) {
    ...
    if (!filtered.length) throw this.app.errors.UNAUTHORISED.setData({ method: req.method, url: req.url })
    return filtered[0]
  }
