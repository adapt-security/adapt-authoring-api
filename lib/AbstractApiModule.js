...
class AbstractApiModule extends AbstractModule {
  ...
  async addRoutes() {
    ...
    this.router.addHandlerMiddleware(this.processRequestMiddleware.bind(this));
  ...
  /**
   * Runs data through the sanitiser to ensure attributes are removed according to the context
  ...
  async sanitise(schemaName, data, options) {
    ...
    const sanitised = await Promise.all((isArray ? data : [data]).map(async d => jsonschema.sanitise(await this.getSchema(schemaName), d, options)));
  ...
  async processRequestMiddleware(req, res, next) {
    ...
    req.apiData = {
      collectionName,
      config,
      data: modifying ? await this.sanitise(schemaName, req.body, { isReadOnly: true }) : { ...req.body },
      modifying,
  ...
  requestHandler() {
    return async (req, res, next) => {
      ...
      try {
        ...
        data = await this.sanitise(req.apiData.schemaName, data, { isInternal: true });
      } catch(e) {
        return next(e);
      }
  ...
  queryHandler() {
    return async (req, res, next) => {
      try {
        ...
        results = await this.sanitise(req.apiData.schemaName, results, { isInternal: true });
