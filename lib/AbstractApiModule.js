...
class AbstractApiModule extends AbstractModule {
  ...
  async serveSchema(req, res, next) {
    try {
      const schema = await this.getSchema(this.schemaName, { compiled: false });
      if(!schema) {
  ...
  async sanitise(schemaName, data, options) {
    ...
    const sanitised = await Promise.all((isArray ? data : [data]).map(async d => {
      return jsonschema.sanitise(await this.getSchema(schemaName, d), d, options);
    }));
  ...
  async sanitiseRequestDataMiddleware(req, res, next) {
    try {
      if(req.apiData.modifying) {
        const schema = await this.getSchema(req.apiData.schemaName, req.apiData.data);
        req.apiData.data = await this.sanitise(schema, req.apiData.data, { isReadOnly: true });
      }
    } catch(e) {
      return next(e);
    }
  ...
  requestHandler() {
    return async (req, res, next) => {
      ...
      try {
        ...
        const schema = await this.getSchema(req.apiData.schemaName, data);
        data = await this.sanitise(schema, data, { isInternal: true, strict: false });
      } catch(e) {
        return next(e);
      }
  ...
  queryHandler() {
    return async (req, res, next) => {
      try {
        ...
        const schema = await this.getSchema(req.apiData.schemaName, results);
        results = await this.sanitise(schema, results, { isInternal: true, strict: false });
  ...
  async update(query, data, options={}, mongoOptions={}) {
    ...
    formattedData.$set = await this.handleValidation({ ...JSON.parse(JSON.stringify(originalDoc)), ...formattedData.$set }, options);
