...
class AbstractApiModule extends AbstractModule {
  ...
  async setUpPagination(req, res, mongoOpts) {
    ...
    const docCount = await mongodb.getCollection(req.apiData.collectionName).countDocuments(req.body);
    const pageTotal = Math.ceil(docCount/pageSize);
    ...
    if(pageTotal > 1) {
      const baseUrl = `${req.originalUrl}${req.originalUrl.includes('?') ? '' : '?'}`;
