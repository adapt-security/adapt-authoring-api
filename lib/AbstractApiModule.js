...
class AbstractApiModule extends AbstractModule {
  ...
  async init() {
    ...
    /**
     * Add data hook listeners
     * Note we wait until app is ready, to allow other modules to add listeners first
     */
  ...
  processRequestMiddleware(config) {
    return (req, res, next) => {
      ...
      // MongoDB throws errors if trying to set the _id manually, so pre-empt that
      delete req.apiData.data._id;
      next();
    };
  }
  /**
  ...
  async handleValidation(oldData, newData, options) {
    if(options.validate) Object.assign(newData, await this.validate(schemaName, newData, options));
  }
  /**
   * Validates data
  ...
  async find(query, options={}, mongoOptions={}) {
    ...
    try {
      if(options.schemaName) { // run a very lax validation (mostly for ObjectId conversion)
        q = await this.validate(options.schemaName, query, { ignoreRequired: true, useDefaults: false, ...options });
      }
  ...
  async update(query, data, options={}, mongoOptions={}) {
    ...
    await this.updateHook.invoke(originalDoc, formattedData.$set, {
      ignoreRequired: true,
      useDefaults: false,
      ...options
    }, mongoOptions);
