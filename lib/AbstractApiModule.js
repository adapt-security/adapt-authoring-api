...
class AbstractApiModule extends AbstractModule {
  ...
  async init() {
     ...
     * @type {Hook}
     */
    this.preInsertHook = new Hook({ type: Hook.Types.Series, mutable: true });
    /**
     ...
     * @type {Hook}
     */
    this.preUpdateHook = new Hook({ type: Hook.Types.Series, mutable: true });
    /**
  ...
  async update(query, data, options={}, mongoOptions={}) {
    ...
    if(options.invokePostHook) this.posUpdateHook.invoke(originalDoc, results);
    return results;
  }
  /**
