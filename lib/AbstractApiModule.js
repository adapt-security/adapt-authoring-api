...
class AbstractApiModule extends AbstractModule {
  ...
  async insert(data, options={}, mongoOptions={}) {
    ...
    options.schemaName = await this.getSchemaName(data);
    await this.preInsertHook.invoke(data, options, mongoOptions);
    
    data = await this.validate(schemaName, data, options);
    const mongodb = await this.app.waitForModule('mongodb');
  ...
  async find(query, options={}, mongoOptions={}) {
    this.setDefaultOptions(options);
    options.schemaName = await this.getSchemaName(query);
    const mongodb = await this.app.waitForModule('mongodb');
    const q = await this.parseQuery(schemaName, query, options, mongoOptions);
  ...
  async update(query, data, options={}, mongoOptions={}) {
    ...
    formattedData.$set = await this.validate(schemaName, {
