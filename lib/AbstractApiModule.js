...
class AbstractApiModule extends AbstractModule {
   ...
   */
  async addRoutes () {
    const config = await server.loadRouteConfig?.(this.rootDir, this, {
      schema: 'apiroutes',
      handlerAliases: {
    ...
    if (config) await this._applyRouteConfig(config)
    if (!this.router) {
      const serverMod = await this.app.waitForModule('server')
      /** @ignore */ this.router = serverMod.api.createChildRouter(this.root)
    }
    const uniqueRoutes = {}
  ...
  async _applyRouteConfig (config) {
    ...
    if (config.useDefaultRoutes !== false) {
      this.routes = [...(await this._getDefaultRoutes()), ...customRoutes]
    } else {
  ...
  /**
   * Returns the default CRUD routes for this module (used when routes.json specifies useDefaultRoutes: true).
   * Loads the route template from default-routes.json, resolves handler aliases, and expands `{scope}` in
   * permission strings with `this.permissionsScope || this.root`.
   * @return {Promise<Array<ApiRoute>>}
   */
  async _getDefaultRoutes () {
    ...
    return template.routes.map(r => {
      ...
      for (const [method, handlerStr] of Object.entries(r.handlers)) {
        resolved.handlers[method] = handlerAliases[handlerStr]
      }
      if (r.permissions) {
        resolved.permissions = {}
        for (const [method, perms] of Object.entries(r.permissions)) {
          resolved.permissions[method] = perms.map(p => p.replace('{scope}', scope))
        }
      }
      return resolved
    })
