...
class AbstractApiModule extends AbstractModule {
  ...
  async addRoutes () {
    try {
      const serverModule = await import('adapt-authoring-server')
      if (typeof serverModule.loadRouteConfig === 'function') {
   ...
   */
  _getDefaultRoutes () {
    const readPerm = `read:${this.permissionsScope || this.root}`
    const writePerm = `write:${this.permissionsScope || this.root}`
    const handler = this.requestHandler()
    const queryH = this.queryHandler()
    return [
      {
        route: '/',
        handlers: { post: handler, get: handler },
        permissions: { post: [writePerm], get: [readPerm] }
      },
      {
        route: '/schema',
        handlers: { get: this.serveSchema.bind(this) }
      },
      {
        route: '/:_id',
        handlers: { put: handler, get: handler, patch: handler, delete: handler },
        permissions: { put: [writePerm], get: [readPerm], patch: [writePerm], delete: [writePerm] }
      },
      {
        route: '/query',
        validate: false,
        modifying: false,
        handlers: { post: queryH },
        permissions: { post: [readPerm] }
      }
    ]
  }
