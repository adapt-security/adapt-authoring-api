...
const DEFAULT_ROUTE_TEMPLATE = require('../routes.json')
/**
...
class AbstractApiModule extends AbstractModule {
   ...
   */
  async addRoutes () {
    if (typeof server.loadRouteConfig === 'function') {
      ...
      if (config) this._applyRouteConfig(config)
    }
    if (!this.router) {
  ...
  _applyRouteConfig (config) {
    ...
    if (config.useDefaultRoutes !== false) {
      this.routes = [...this._getDefaultRoutes(), ...customRoutes]
    } else {
   ...
   * Loads the route template from routes.json, resolves handler aliases, and expands `{scope}` in
  ...
  _getDefaultRoutes () {
    ...
    return DEFAULT_ROUTE_TEMPLATE.routes.map(r => {
