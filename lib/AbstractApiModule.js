...
class AbstractApiModule extends AbstractModule {
  ...
  async init() {
    ...
    /**
     * Hook invoked just before data is returned, to give the opportunity to sanitise any data prior to returning via the web API
     * @type {Hook}
     */
     this.sanitiseDataHook = new Hook({ mutable: true });
    /**
  ...
  requestHandler() {
    return async (req, res, next) => {
      ...
      try {
        ...
        await this.sanitiseDataHook.invoke(data);
      } catch(e) {
        return next(e);
      }
  ...
  async replace(query, data, options={}, mongoOptions={}) {
    ...
    await this.preUpdateHook.invoke(originalDoc, data, options, mongoOptions);
    data = await this.handleValidation(data, options);
