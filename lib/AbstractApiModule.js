...
class AbstractApiModule extends AbstractModule {
  ...
  useDefaultRouteConfig() {
    ...
    // the below 'meta' vars are metadata for the REST API docs
    const metaParams = [
      { name: 'limit', in: 'query', description: `How many results should be returned Default value is ${this.app.config.get('adapt-authoring-api.defaultPageSize')} (max value is ${this.app.config.get('adapt-authoring-api.maxPageSize')})` },
      { name: 'page', in: 'query', description: 'The page of results to return (determined from the limit value)' }
    ];
    const metaSchemaRef = `#/components/schemas/${this.schemaName}`;
    const metaData = {
      description: `The ${this.schemaName} data`,
      content: { "application/json": { schema: { $ref: metaSchemaRef } } }
    };
    const metaListData = {
      description: `List of ${this.collectionName}`,
      content: { "application/json": { schema: { type: "array", items: { $ref: metaSchemaRef } } } }
    };
    /** @ignore */ this.routes = [
      {
        route: '/',
        ...
        meta: {
          post: { summary: `Create a new ${this.schemaName}`, requestBody: metaData, responses: { 201: metaData } },
          get: { summary: `Retrieve all ${this.collectionName}`, parameters: metaParams, responses: { 200: metaListData } }
        },
      },
      {
        route: '/schema',
        ...
        meta: {
          get: { summary: `Retrieve ${this.schemaName} schema` }
        },
      },
      {
        route: '/:_id',
        modifiers: ['put', 'patch', 'delete'],
        ...
        meta: {
          put: { summary: `Replace an existing ${this.schemaName} document`, requestBody: metaData, responses: { 200: metaData } },
          get: { summary: `Retrieve a single ${this.schemaName} document`, responses: { 200: metaData } },
          patch: { summary: `Update an existing ${this.schemaName} document`, requestBody: metaData, responses: { 200: metaData } },
          delete: { summary: `Delete an existing ${this.schemaName} document`, responses: { 204: {} } }
        }
      },
      {
        route: '/query',
        ...
        meta: {
          post: {  summary: `Query all ${this.collectionName}`, parameters: metaParams, responses: { 200: metaListData } }
        }
      }
    ];
  }
