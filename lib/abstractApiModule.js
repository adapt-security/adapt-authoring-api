...
const { App, AbstractModule, Responder } = require('adapt-authoring-core');
...
class AbstractApiModule extends AbstractModule {
  ...
  */
  queryHandler() {
    return async(req, res, next) => {
      try {
        const query = await this.validate(req.apiData.schemaName, req.body, { useDefaults: false, ignoreRequired: true });
  ...
  */
  async validateMiddleware(req, res, next) {
    if(req.apiData.config.validate === false) {
      return next();
    }
    const v = (data, lite=false) => {
      const opts = { useDefaults: !lite, ignoreRequired: lite };
      return this.validate(req.apiData.schemaName, data, opts);
    };
    if(req.hasQuery) {
      try {
        req.apiData.query = await v(req.apiData.query, true);
      } catch(e) {
        next(new Error(`Request query failed validation: ${e.errors.join(', ')}`));
      }
    }
    if(req.hasBody) {
      try {
        ...
        next(new Error(`Request body failed validation: ${e.errors.join(', ')}`));
      }
    }
    next();
  }
