...
class AbstractApiModule extends AbstractModule {
  ...
  */
  queryHandler() {
    return async(req, res, next) => {
      try {
        const query = await this.validate(req.apiData.schemaName, req.body, {
          useDefaults: false,
          ignoreRequired: true
        });
  ...
  async validateMiddleware(req, res, next) {
    ...
    if(req.hasQuery) {
      try {
        ...
        return next(new Error(`Request query failed validation: ${e.errors.join(', ')}`));
      }
    }
    if(req.hasBody) {
      try {
        ...
        return next(new Error(`Request body failed validation: ${e.errors.join(', ')}`));
      }
    }
    next();
  }
  ...
  */
  async validate(schemaName, data, options) {
    ...
    return jsonschema.validate(schemaName, data, options);
  }
  /**
