...
class AbstractApiModule extends AbstractModule {
  ...
  */
  ...
  validateValues() {
    if(!Utils.isObject(def)) {
      throw new Error(this.app.lang.t('notobject', { def: typeof def }));
    }
    if(!this.name) {
      throw new Error(this.app.lang.t('noname'));
    }
    if(!this.routes) {
      throw new Error(this.app.lang.t('noroutes'));
    }
  }
  /**
  ...
  */
  ...
  addRoute(config) {
    Object.entries(r.handlers).forEach(([key, handler]) => {
      const handlers = Utils.isArray(handler) ? [...handler] : [handler];
      config.handlers = [...middleware, ...handlers];
    }, {});
