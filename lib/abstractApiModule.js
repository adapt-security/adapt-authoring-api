  ...
  async validateMiddleware(req, res, next) {
    ...
    const v = async (dataKey, lite = false) => {
      try {
        ...
        return jsonschema.validate(schema, req.apiData[dataKey], { useDefaults: !lite, ignoreRequired: lite });
      } catch(e) {
  ...
  */
  queryHandler() {
    return async(req, res, next) => {
      try {
        const jsonschema = await this.app.waitForModule('jsonschema');
        const validated = await jsonschema.validate(req.apiData.schema, req.body, { useDefaults: false, ignoreRequired: true });
