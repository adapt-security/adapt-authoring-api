...
class AbstractApiModule extends AbstractModule {
  ...
  */
  requestHandler() {
    return async (req, res, next) => {
      ...
      let data;
      try {
        const isFind = method === 'get';
        if(!isFind) await this.checkAccess(req, query);
        data = await func.apply(this, ApiUtils.argsFromReq(req));
        if(isFind) data = await this.checkAccess(req, data);
      } catch(e) {
        return next(e);
      }
  ...
  */
  ...
  async checkAccess(req, data) {
    ...
    await Promise.allSettled((isArray ? data : [data]).map(async r => {
      try {
        const hasAccess = (await this.accessCheckHook.invoke(req, r)).some(Boolean);
        if(hasAccess) filtered.push(r);
      } catch(e) {
        error = AuthError.Authorise(`user does not have access to this resource`);
      }
    }));
  ...
  */
  async update(query, data, options={}, mongoOptions={}) {
    ...
    const mongodb = await this.app.waitForModule('mongodb');
    return mongodb.update(options.collectionName, query, { $set: validatedData }, mongoOptions);
  }
  /**
