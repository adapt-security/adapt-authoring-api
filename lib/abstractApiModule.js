...
class AbstractApiModule extends AbstractModule {
  ...
  async _init() {
    ...
    this.insertHook.tap((newD, opts, mOpts) => this.handleValidation(undefined, newD, opts, mOpts));
    this.updateHook.tap((oldD, newD, opts, mOpts) => this.handleValidation(oldD, newD, opts, mOpts));
  }
  /**
  ...
  */
  ...
  async handleValidation(originalData, data, options) {
    if(options.validate) Object.assign(data, await this.validate(options.schemaName, data));
  }
  /**
  * Validates data
