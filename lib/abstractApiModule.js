...
class AbstractApiModule extends AbstractModule {
  ...
  */
  addRoute(config, auth) {
    Object.entries(config.handlers).forEach(([method, handler]) => {
      ...
      auth.permissions.secureRoute(`${this.router.path}${route}`, method, perms);
    }, {});
  ...
  */
  async insert(data, options={}, mongoOptions={}) {
    ...
    const results = await mongodb.insert(options.collectionName, validatedData, mongoOptions);
  ...
  */
  async update(query, data, options={}, mongoOptions={}) {
    ...
    if(!originalDoc) throw new Error('no matching document found');
    const results = await mongodb.update(options.collectionName, query, { $set: validatedData }, mongoOptions);
  ...
  */
  async replace(query, data, options={}, mongoOptions={}) {
    ...
    const [originalDoc] = await mongodb.find(options.collectionName, query);
    if(!originalDoc) throw new Error('no matching document found');
    const results = await mongodb.replace(options.collectionName, query, validatedData, mongoOptions);
  ...
  */
  async delete(query, options={}, mongoOptions={}) {
    ...
    const [originalDoc] = await mongodb.find(options.collectionName, query);
    if(!originalDoc) throw new Error('no matching document found');
    const results = await mongodb.delete(options.collectionName, query, mongoOptions);
