...
class AbstractApiModule extends AbstractModule {
  ...
  */
  static processRequestDataMiddleware(config) {
  ...
  */
  async validateMiddleware(req, res, next) {
    ...
    let schema;
    try {
      schema = this.getSchema(req.apiData.schemaName);
    } catch(e) {
      return next(new Error(`Failed to retrieve schema, ${e}`));
    }
    const v = (data, lite = false) => {
      const opts = { useDefaults: !lite, ignoreRequired: lite };
      return this.validate(schema, data, opts);
    };
    const errors = [];
    if(req.hasQuery) {
      try {
  ...
  */
  queryHandler() {
    return async(req, res, next) => {
      try {
        const validated = await this.validate(req.apiData.schema, req.body, { useDefaults: false, ignoreRequired: true });
        const results = await this.find(req.apiData.collectionName, validated);
