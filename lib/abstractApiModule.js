  ...
  async validateMiddleware(req, res, next) {
    ...
    const v = async (dataKey, lite = false) => {
      try {
        ...
        return this.validate(schema, req.apiData[dataKey], { useDefaults: !lite, ignoreRequired: lite });
      } catch(e) {
        errors.push(new Error(`${dataKey}: ${e}`));
      }
    };
  ...
  requestHandler() {
    return async (req, res, next) => {
      ...
      if(_.isArray(data) && req.params._id) { // special case for when _id param is present
        if(!data.length) {
          const message = this.app.lang.t('error.itemnotfound');
          this.log('error', message);
          return r.error(message, Codes.Error.Missing);
        }
        data = data[0];
