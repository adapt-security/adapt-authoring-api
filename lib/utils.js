  ...
  */
  static callDbFunction(req) {
    return new Promise((resolve, reject) => {
      ...
      mdb[funcName](...args)
        .then(data => {
          const hasResults = Utils.isArray(data) && data.length > 0 && data.result && data.result.n > 0;
          console.log(funcName, args, hasResults, data);
          if(req.params._id && !hasResults) {
            return reject(formatError('dbfail', 'itemnotfound', { type: req.type }, 404));
          }
          resolve(data);
        })
        .catch(e => reject(formatError('dbfail', e.message)));
    });
  }
  /**
